...
sales_featured as (
  select 
    ...
    n1d.iop as next_1d_is_on_promo,
    n2d.iop as next_2d_is_on_promo,
    n3d.iop as next_3d_is_on_promo,
    n4d.iop as next_4d_is_on_promo,
    n5d.iop as next_5d_is_on_promo,
    n6d.iop as next_6d_is_on_promo,
    n7d.iop as next_7d_is_on_promo,
    ...
    -- l2d.units_sold
  from sales_snapshot as ss
  left join next_sales as n1d on n1d.n1d_dt = dt and n1d.pid = ss.pid and n1d.sid = ss.sid
  left join next_sales as n2d on n2d.n2d_dt = dt and n2d.pid = ss.pid and n2d.sid = ss.sid
  left join next_sales as n3d on n3d.n3d_dt = dt and n3d.pid = ss.pid and n3d.sid = ss.sid
  left join next_sales as n4d on n4d.n4d_dt = dt and n4d.pid = ss.pid and n4d.sid = ss.sid
  left join next_sales as n5d on n5d.n5d_dt = dt and n5d.pid = ss.pid and n5d.sid = ss.sid
  left join next_sales as n6d on n6d.n6d_dt = dt and n6d.pid = ss.pid and n6d.sid = ss.sid
  left join next_sales as n7d on n7d.n7d_dt = dt and n7d.pid = ss.pid and n7d.sid = ss.sid
